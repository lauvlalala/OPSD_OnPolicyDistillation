#!/bin/bash
# Train OPSD on ALFWorld (multi-turn with environment tool)
#
# Prerequisites:
...
#
# Usage:
#   MODEL_PATH=/path/to/model bash scripts/opd/train_opsd_alfworld.sh
#
set -x
...
TOOL_CONFIG_PATH="${SRC_ROOT}/tools/alfworld/tool_config.yaml"

DATA_DIR=${DATA_DIR:-"${REPO_ROOT}/data/alfworld"}

if [ ! -f "${DATA_DIR}/train.parquet" ]; then
    echo "Preparing ALFWorld data..."
    python3 "${SRC_ROOT}/data/prepare_alfworld.py" \
        --output-dir "$DATA_DIR" \
...
EXP_NAME=${MODEL_NAME_SAFE}-OPSD-alfworld-rollout-ema${teacher_ema_decay}
OUTPUT_ROOT=${OUTPUT_ROOT:-"${REPO_ROOT}/outputs"}
output_dir="${OUTPUT_ROOT}/${EXP_NAME}"
mkdir -p "$output_dir"

echo "=== OPSD Training (ALFWorld, multi-turn) ==="
echo "MODEL: $MODEL_PATH | PI: rollout | TOOL: alfworld"
...
python3 -m opd.main_opd \
    ...
    reward.custom_reward_function.path="${SRC_ROOT}/rewards/alfworld_reward.py" \
