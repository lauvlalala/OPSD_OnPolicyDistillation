#!/bin/bash
# Train OPSD on SearchQA (Search-R1 style, multi-turn with search tool)
#
# Prerequisites:
#   1. Start retrieval server: bash src/tools/search/start_retrieval_server.sh
#   2. Prepare data: python -m data.prepare_searchqa --output-dir data/searchqa
#
# Usage:
#   MODEL_PATH=/path/to/model bash scripts/opd/train_opsd_searchqa.sh
#
set -x
...
TOOL_CONFIG_PATH="${SRC_ROOT}/tools/search/tool_config.yaml"

DATA_DIR=${DATA_DIR:-"${REPO_ROOT}/data/searchqa"}
TRAIN_FILE="${DATA_DIR}/train.parquet"
VAL_FILE="${DATA_DIR}/test.parquet"

MODEL_NAME_SAFE=$(echo "$MODEL_NAME" | tr '/' '_')
EXP_NAME=${MODEL_NAME_SAFE}-OPSD-searchqa-rollout-ema${teacher_ema_decay}
OUTPUT_ROOT=${OUTPUT_ROOT:-"${REPO_ROOT}/outputs"}
output_dir="${OUTPUT_ROOT}/${EXP_NAME}"
mkdir -p "$output_dir"

echo "=== OPSD Training (SearchQA, multi-turn) ==="
echo "MODEL: $MODEL_PATH | PI: rollout | TOOL: search"
...
python3 -m opd.main_opd \
    ...
    reward.custom_reward_function.path="${SRC_ROOT}/rewards/searchqa_reward.py" \
