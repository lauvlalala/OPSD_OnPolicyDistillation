#!/bin/bash
# Train OPSD on DAPO-17k (self-distillation with PI + EMA)
#
# Usage:
#   MODEL_PATH=/path/to/model bash scripts/opd/train_opsd_dapo.sh
#
set -x
...
if [ ! -f "${DATA_DIR}/train.parquet" ]; then
    ...
    python3 "${SRC_ROOT}/data/prepare_dapo.py" \
        --output-dir "$DATA_DIR"
fi

TRAIN_FILE="${DATA_DIR}/train.parquet"

# Validation/evaluation: standard math benchmarks (same as reference repo train_opd.sh)
EVAL_DIR=${EVAL_DIR:-"${REPO_ROOT}/data/grpo_processed"}
VAL_MATH500="${EVAL_DIR}/val_math500.parquet"
VAL_AIME24="${EVAL_DIR}/val_aime24.parquet"
VAL_AIME25="${EVAL_DIR}/val_aime25.parquet"
...
EXP_NAME=${MODEL_NAME_SAFE}-OPSD-dapo-${opd_loss_type}-ema${teacher_ema_decay}
OUTPUT_ROOT=${OUTPUT_ROOT:-"${REPO_ROOT}/outputs"}
output_dir="${OUTPUT_ROOT}/${EXP_NAME}"
mkdir -p "$output_dir"

echo "=== OPSD Training (DAPO-17k) ==="
...
echo "=== OPSD training (DAPO) completed ==="
