#!/bin/bash
# Train OPSD on ToolUse (SDPO-style: rollout PI + EMA)
#
# Usage:
#   MODEL_PATH=/path/to/model TOOLUSE_DIR=/path/to/tooluse bash scripts/opd/train_opsd_tooluse.sh
#
set -x
...
TOOLUSE_DIR=${TOOLUSE_DIR:-"${REPO_ROOT}/data/tooluse"}
...
DATA_DIR=${DATA_DIR:-"${REPO_ROOT}/data/tooluse"}

if [ ! -f "${DATA_DIR}/train.parquet" ]; then
    echo "Preparing ToolUse data..."
    python3 "${SRC_ROOT}/data/prepare_tooluse.py" \
...
EXP_NAME=${MODEL_NAME_SAFE}-OPSD-tooluse-rollout-ema${teacher_ema_decay}
OUTPUT_ROOT=${OUTPUT_ROOT:-"${REPO_ROOT}/outputs"}
output_dir="${OUTPUT_ROOT}/${EXP_NAME}"
mkdir -p "$output_dir"

echo "=== OPSD Training (ToolUse) ==="
...
python3 -m opd.main_opd \
    ...
    reward.custom_reward_function.path="${SRC_ROOT}/rewards/tooluse_reward.py" \
